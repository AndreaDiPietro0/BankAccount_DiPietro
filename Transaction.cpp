...
Transaction::Transaction(double amt, Type t, std::string desc) : amount(amt), type(t), description(std::move(desc)) {
    ...
    ss << std::put_time(std::localtime(&now_c),
                        "%Y-%m-%d %H:%M:%S"); //prende l'ora e separa tempo in a,m,g..., put time ordina i valori in Y-M... e mette in ss
